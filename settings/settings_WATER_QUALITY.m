...
load('GBR_REEF_POP.mat') % Baseline scenarios (6 years of demographic layers)
...
for yr = 1:size(GBR_REEF_POP,2) % for every year
    ...
    REEF_POP(yr).COTS_larvae_survival(REEF_POP(yr).COTS_larvae_survival<0.01)=0.01; % note: if 0.001 needs a beta = 0.5*1e6 to fit with Lizard data
