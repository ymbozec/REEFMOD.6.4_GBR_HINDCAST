...
META.COTS_outbreak_duration = [2,5] ; % min and max duration of an outbreak (random duration within this range)
% META.COTS_outbreak_duration = 2 ; % fixed duration to be used to reproduce Lizard outbreak (Pratchett)
META.COTS_density_threshold_for_disease = 0.6 ;  % per 400m2
% A CoTS population is considered outbreaking when the density of 18 mo+ starfish reaches 0.3 individuals per 200 m2 
% (Keesing and Lucas 1992, Moran and De’Ath 1992). If COTS density is above this threshold for the specified duration
% then population crashes due to disease  

META.COTS_background_density = 0 ; %total density (nb CoTS per grid) of CoTS, excluding recruits
...
% For Karlo's code in COTS_control (1 if META.COTS_min_selfseed>0):
META.force_COTS_selfseed = 1 ; % 

% Minimum proportional coral cover below which CoTS population crashes (end of the outbreak)
META.COTS_coral_threshold = 0.05 ; % as total of corals #1 to #3 (set below)

% ID of preferred corals to evaluate the coral threshold
META.COTS_pref_corals = [1:3] ; % as total of corals #1 to #3
