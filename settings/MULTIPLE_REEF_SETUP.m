...
for n = 1:length(META.reef_ID)
    ...
    if META.doing_genetics == 0
        ...
        REEF(n).predicted_DHWs = squeeze(DHW(n,1:META.nb_time_steps))+ RESTORATION.doing_cooling*RESTORATION.cooling*12;
        REEF(n).Topt_baseline = [];
        REEF(n).predicted_SST = zeros(1,META.nb_time_steps);
        
    else
        REEF(n).predicted_DHWs = squeeze(DHW(n,1:META.nb_time_steps,:))'+ RESTORATION.doing_cooling*RESTORATION.cooling*12; % with all the Topt stacks
        REEF(n).Topt_baseline = MMM_CoRTAD(META.reef_ID(n),1) - META.Topt_offset ;
        REEF(n).predicted_SST = SST_GBR(n,:) + RESTORATION.doing_cooling*RESTORATION.cooling/2;  % Decreases SST for only 3 months in summer
