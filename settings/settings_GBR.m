...
relative_cover = table2array(outerjoin(GBR_REEFS,AIMS_AVERAGECOMPO, 'LeftKeys', [9 10],'RightKeys', [2 1], 'LeftVariables',[4 9 10], 'RightVariables',[4:9]));
...
NB_REEFS_TR_INIT = length(unique(AIMS_TR.KarloID)); %123 reefs surveyed using transects between 2006-2008
...
NB_REEFS_MT_INIT = length(unique(AIMS_MT.KarloID)); %140 reefs surveyed using transects between 2006-2008
NB_REEFS_TOT_INIT = length(unique([unique(AIMS_TR.KarloID) ; unique(AIMS_MT.KarloID)])); % 186 reefs in total used for intitialisation of coral cover
...
AIMS_ALL = varfun(omean, AIMS_ALLtmp,'GroupingVariables',...
    {'KarloID';'SECTOR';'AIMS_SHELF'}, 'InputVariables','CCOVER'); %average across sites/method
...
init_coral = table2array(outerjoin(GBR_REEFS,AIMS_AVERAGESTATE, 'LeftKeys', [9 10],'RightKeys', [2 1], 'LeftVariables',[4 9 10], 'RightVariables',[4]));
...
init_coral(AIMS_ALL.KarloID,4) = AIMS_ALL.Fun_CCOVER;
...
GBR_PAST_COTS_PER_GRID = (GBR_PAST_COTS_NEW/0.22)*(1500/2500); % Conversion: 1500 COTS per km2 ~0.22 per manta tow (Moran and De'ath 1986)
...
%% PAST STORM REGIME 2008 to 2017
...
% FUTURE_CYCLONE_CAT(:,1:2:end) = Cyc_cat(GBR_REEFS.NickID,:,simul);
% 
