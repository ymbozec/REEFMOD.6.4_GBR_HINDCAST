...
function [coral,algal,new_density_COTS,total_coral_loss] = f_apply_COTS_predation(coral, algal, density_COTS, density_COTS_settlers, COTS_feeding_rates, SI, META)
...
if sum(new_density_COTS(2:end))>0 && sum(coral_species_planar_area_cm2(1,META.COTS_pref_corals))/META.total_area_cm2 < META.COTS_coral_threshold    
%     new_density_COTS(4:end) = 0 ;
%     new_density_COTS(2:end) = 0 ; % now also kills juveniles
    new_density_COTS(2:end) = META.COTS_background_density * new_density_COTS(2:end)/sum(new_density_COTS(2:end)) ; % now also kills juveniles
